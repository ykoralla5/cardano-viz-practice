...
# Helper function
def get_delegator_movements_granular(epoch_number):
    """
    ...
    nearest_lower_stake_qs = models.MvEpochPoolStats.objects \
    ...
    missing_id_stakes_qs = models.MvEpochPoolStats.objects \
...
def get_delegator_movement_counts_percents(epoch_number):
    """
    Get effective delegator movement counts and amounts by stake change percents from MV.
    """
    start_time = time.time()
    ...
    logger.info("Took " + str(time.time() - start_time) + " seconds to run get_delegator_movement_counts_percents helper function.")
    print("Took " + str(time.time() - start_time) + " seconds to run get_delegator_movement_counts_percents helper function.")
...
def get_pool_stats(epoch_number, delegation_ids, pool_stakes):
    ...
    pool_stats_qs = models.MvEpochPoolStats.objects \
            .filter(epoch_no=epoch_number) \
    ...
    if missing_ids:
        ...
        pool_views = {
            ...
            for pool_id, pool_view, pledge in models.MvEpochPoolStats.objects.filter(pool_id__in=missing_ids).order_by('pool_id', '-epoch_no').values_list('pool_id', 'pool_view', 'pledge') }
...
def get_epoch_params(epoch_number):
    ...
    logger.info("Took " + str(time.time() - start_time) + " seconds to run get_epoch_params helper function.")
    print("Took " + str(time.time() - start_time) + " seconds to run get_epoch_params helper function.")
...
def get_epoch_snapshot(request):
    ...
    if epoch_qparam:
        ...
        if isinstance(epoch_number, JsonResponse):
            logger.info("Running get_epoch_snapshot for epoch " + epoch_number)
            return epoch_number
    else:
    ...
    delegations_data, delegations_ids, pool_stakes = get_delegator_movements_granular(epoch_number)
