...
// const generateEpochs = (start, end) => {
//     const epochs = []
//     if (start > end) return epochs
//     return Array.from({ length: end - start + 1 }, 
//                      (_, index) => start + index).reverse()
// }
                        ...
                        <table className="min-w-full border border-gray-300 dark:border-gray-600">
                            <thead className="sticky top-0 bg-gray-800 z-10">
                                <tr className="text-left text-gray-700 dark:text-gray-300">
                                    <th className="border border-gray-300 dark:border-gray-600 px-2 py-1 w-15">Epoch</th>
                                    <th className="border border-gray-300 dark:border-gray-600 px-2 py-1">Start time to End time</th>
                                </tr>
                            </thead>
                            <tbody>
                                {epochsList.map(([e, data], index) => (
                                    <tr onClick={() => setFilters(prev => ({ ...prev, epoch: e }))} 
                                        ...
                                        className={`text-gray-700 dark:text-white cursor-pointer ${index % 2 === 0 ? "bg-gray-50 dark:bg-gray-800" : "bg-white dark:bg-gray-700"} ${filters.epoch === e ? "outline outline-2 outline-teal-400" : ""} hover:bg-teal-500 hover:text-black`}>
                                            ...
                                            <td className="border border-gray-300 dark:border-gray-600 px-2 py-1 font-bold text-sm">{data.start_time} to {data.end_time}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
